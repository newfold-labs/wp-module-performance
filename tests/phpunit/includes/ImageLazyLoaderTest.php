...
class ImageLazyLoaderTest extends TestCase {
	...
	public function test_image_lazy_loader() {
		WP_Mock::expectActionAdded(
			'wp_enqueue_scripts',
			array( WP_Mock\Functions::type( ImageLazyLoader::class ), 'enqueue_lazy_loader' )
		);
		...
		foreach ( $filters_to_mock as $filter ) {
			WP_Mock::expectFilterAdded(
				$filter,
				array( WP_Mock\Functions::type( ImageLazyLoader::class ), 'apply_lazy_loading' )
			);
		}

		WP_Mock::expectFilterAdded(
			'render_block',
			array( WP_Mock\Functions::type( ImageLazyLoader::class ), 'apply_lazy_loading_to_blocks' ),
			10,
			2
		);
